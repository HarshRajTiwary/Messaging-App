...
class ChatApp:
    def __init__(self, master):
        ...
        # Connection status
        self.status_label = tk.Label(master, text="Not Connected", bg="red", fg="white", width=20)
        self.status_label.grid(row=2, column=2, padx=10, pady=10)

        # Serial port and threading
    ...
    def connect_to_port(self, event=None):
        ...
        try:
            ...
            self.status_label.config(text="Connected", bg="green", fg="white")
            self.running = True
            ...
            self.status_label.config(text="Connection Failed", bg="red", fg="white")
    ...
    def send_message(self, event=None):
        ...
        if self.serial_port and self.serial_port.is_open:
            ...
            self.chat_log.config(state='normal')
            self.chat_log.insert(tk.END, "Error: Not connected to any device.\n")
            self.chat_log.config(state='disabled')
            self.chat_log.yview(tk.END)
        ...
        while self.running:
            if self.serial_port and self.serial_port.in_waiting > 0:
                try:
                    ...
                    self.serial_port = None
                    self.status_label.config(text="Not Connected", bg="red", fg="white")
                    self.running = False
                    break
